...
def get_messages(queue):
    ...
    while True:
        for message in queue.receive_messages(
            MessageAttributeNames=["userId", "imageUrl"], MaxNumberOfMessages=10
        ):
            ...
            if not message or not message.body:
                ...
                yield Message(
                    text=message.body,
                    user=message.message_attributes.get("userId").get("StringValue"),
                    imageUrl=message.message_attributes.get("imageUrl").get(
                        "StringValue"
                    ),
                    message=message,
                )
...
def imageUrl_to_s3_path(imageUrl, imageNum):
    # "https://story-images.s3.eu-central-1.amazonaws.com/reimagine/skajd/images/5d41402abc4b2a76b9719d911017c592-0.png"
    print("in imageurl to s3 path: ", imageUrl)
    print(f'{imageUrl.split("reimagine/")[1]}')
    return f'reimagine/{imageUrl.split("reimagine/")[1].split("-")[0]}-{imageNum}.png'


def imageUrl_to_image_id(imageUrl):
    return imageUrl.split("/")[-1].split("-")[0]


def upload_images(image_paths, s3, imageUrl):
    s3_paths = []
    for i, path in enumerate(image_paths):
        # 1. upload to s3
        s3_path = imageUrl_to_s3_path(imageUrl, i)
...
def main():
    ...
    for i in get_messages(queue=queue):
        ...
        print(f"Recieved message: {i}")
        story_path = base_path / f"reimagine/{i.user}/images"
        # 3. process messages

        model = init_model(model_to_use, last_used_model, model)

        image_paths = model(i.text, story_path, imageUrl_to_image_id(i.imageUrl))
        last_used_model = model_to_use

        # 4. upload each image to s3
        s3_paths = upload_images(image_paths, s3, i.imageUrl)

        # 5. make the images publicly accessible
        set_public_tag(s3_paths, s3)
        print(f"Finished processing message for url: {i.imageUrl}")
